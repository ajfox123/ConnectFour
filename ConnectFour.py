...
class ConnectFour():
    ...
    def get_valid_locations(self, board_array):
        valid_locations = []
        for col in range(7):
            if self.is_valid_drop(board_array[5], col):
    ...
    def min_max(self, d, board, first_player):
        ...
        if first_player:
            ...
            for col in valid_locations:
                ...
                curr_score_2 = board.score_r if self.turn_1 == 'y' else board.score_y
                was_won = board.won != ''
                board.drop_piece(row, col, self.turn_1)
                m = self.min_max(d + 1, board, False)[1]
                board.board[row, col] = '.'
                if board.won != '':
                    board.won = ''
            ...
            for col in valid_locations:
                ...
                m = self.min_max(d + 1, board, True)[1]
                board.board[row, col] = '.'
                board.score_r = curr_score_1 if self.turn_1 == 'r' else curr_score_2
                board.score_y = curr_score_1 if self.turn_1 == 'y' else curr_score_2
                if not was_won:
